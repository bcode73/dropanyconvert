...
function renderHead(seo, config, toolIndex, opts = {}) {
  ...
  // AdSense account meta tag (every page, including legal — required by Google)
  const adsenseMeta = (ads?.clientId && !ads.clientId.includes('XXXX'))
    ? `<meta name="google-adsense-account" content="${ads.clientId}">`
    : '';

  // AdSense publisher script (non-legal pages only)
